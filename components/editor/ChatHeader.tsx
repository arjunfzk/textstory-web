/**
 * Chat header bar for the phone preview.
 *
 * Renders style-specific headers:
 * - iMessage: white bg, blue "‹" back, centered name, FaceTime icon
 * - WhatsApp: dark green bg, back arrow + avatar + name/online, call icons
 * - Instagram: white bg, back arrow, avatar + name, call icons
 */

'use client';

import type { ChatContact, ChatStyle, StyleTokens } from '@/lib/types';

interface ChatHeaderProps {
  contact: ChatContact;
  tokens: StyleTokens;
  style: ChatStyle;
}

export function ChatHeader({ contact, tokens, style }: ChatHeaderProps) {
  if (style === 'whatsapp') return <WhatsAppHeader contact={contact} tokens={tokens} />;
  if (style === 'instagram') return <InstagramHeader contact={contact} tokens={tokens} />;
  return <IMessageHeader contact={contact} tokens={tokens} />;
}

/** iMessage-style header: white bg, blue back chevron, centered name. */
function IMessageHeader({ contact, tokens }: { contact: ChatContact; tokens: StyleTokens }) {
  return (
    <div
      className="relative flex items-center justify-center px-4 py-2.5"
      style={{ fontFamily: tokens.fontFamily }}
    >
      {/* Left: back chevron + "Messages" */}
      <div className="absolute left-3 flex items-center gap-0.5">
        <span className="text-[#007AFF] text-xl font-light">‹</span>
        <span className="text-[#007AFF] text-[15px]">Messages</span>
      </div>

      {/* Center: name */}
      <span className="font-semibold text-[15px] text-black">{contact.name}</span>

      {/* Right: FaceTime icon */}
      <div className="absolute right-3 flex items-center gap-3">
        <svg width="20" height="16" viewBox="0 0 20 16" fill="none">
          <rect x="0.5" y="2" width="13" height="12" rx="2" stroke="#007AFF" strokeWidth="1.5" />
          <path d="M15 6L19 3.5V12.5L15 10V6Z" stroke="#007AFF" strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
      </div>
    </div>
  );
}

/** WhatsApp-style header: dark green bg, avatar, name + "online", call icons. */
function WhatsAppHeader({ contact, tokens }: { contact: ChatContact; tokens: StyleTokens }) {
  return (
    <div
      className="flex items-center gap-2.5 px-2 py-2"
      style={{ color: tokens.headerText, fontFamily: tokens.fontFamily }}
    >
      {/* Back arrow */}
      <span className="text-white text-xl font-light">‹</span>

      {/* Avatar */}
      <div className="w-9 h-9 rounded-full overflow-hidden shrink-0 bg-[#DFE5E7]">
        {contact.profileImageUrl ? (
          <img src={contact.profileImageUrl} alt="" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="#FFFFFF">
              <path d="M12 12c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm0 2c-3.33 0-10 1.67-10 5v2h20v-2c0-3.33-6.67-5-10-5z" />
            </svg>
          </div>
        )}
      </div>

      {/* Name + online */}
      <div className="flex-1 min-w-0">
        <div className="font-medium text-[15px] text-white truncate">{contact.name}</div>
        <div className="text-[12px] text-white/70">online</div>
      </div>

      {/* Call icons */}
      <div className="flex items-center gap-4 mr-1">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
          <path d="M17 2H7C5.9 2 5 2.9 5 4V20C5 21.1 5.9 22 7 22H17C18.1 22 19 21.1 19 20V4C19 2.9 18.1 2 17 2ZM12 17.5C11.17 17.5 10.5 16.83 10.5 16C10.5 15.17 11.17 14.5 12 14.5C12.83 14.5 13.5 15.17 13.5 16C13.5 16.83 12.83 17.5 12 17.5ZM15.5 13H8.5V4.5H15.5V13Z" />
        </svg>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
          <path d="M6.62 10.79a15.91 15.91 0 006.59 6.59l2.2-2.2a1 1 0 011.01-.24 11.72 11.72 0 003.68.59 1 1 0 011 1V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 011 1 11.72 11.72 0 00.59 3.68 1 1 0 01-.25 1.02l-2.22 2.09z" />
        </svg>
      </div>
    </div>
  );
}

/** Instagram DM-style header: white bg, back arrow, avatar + name, icons. */
function InstagramHeader({ contact, tokens }: { contact: ChatContact; tokens: StyleTokens }) {
  return (
    <div
      className="flex items-center gap-3 px-3 py-2.5"
      style={{ color: tokens.headerText, fontFamily: tokens.fontFamily }}
    >
      {/* Back arrow */}
      <span className="text-[#262626] text-xl font-light">‹</span>

      {/* Avatar */}
      <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 border border-gray-200">
        {contact.profileImageUrl ? (
          <img src={contact.profileImageUrl} alt="" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-purple-400 to-pink-400 text-white text-xs font-bold">
            {contact.name.charAt(0).toUpperCase()}
          </div>
        )}
      </div>

      {/* Name + Active */}
      <div className="flex-1 min-w-0">
        <div className="font-semibold text-sm text-[#262626] truncate">{contact.name}</div>
        <div className="text-[11px] text-gray-500">Active now</div>
      </div>

      {/* Call + video icons */}
      <div className="flex items-center gap-4 mr-1">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#262626" strokeWidth="2">
          <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z" />
        </svg>
        <svg width="20" height="16" viewBox="0 0 20 16" fill="none">
          <rect x="0.5" y="2" width="13" height="12" rx="2" stroke="#262626" strokeWidth="1.5" />
          <path d="M15 6L19 3.5V12.5L15 10V6Z" stroke="#262626" strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
      </div>
    </div>
  );
}
